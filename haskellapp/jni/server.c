...
HugsServerAPI* initHugsServer(argc, argv) /*server initialisation*/
Int    argc;
String argv[]; {
    ...
    if (!is_initialized) {
      ...
      if (argc < 0) {
	//readOptions(argv[0],FALSE); //perceber o pq de não funcionar
      } else {
	readOptionSettings();
 ...
 * Shutting down:
 * ------------------------------------------------------------------------*/
void shutdownHugsServer(hserv) /* server shutdown */
HugsServerAPI* hserv; {
